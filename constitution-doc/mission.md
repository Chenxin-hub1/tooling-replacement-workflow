...
3. 旧模处置闭环断裂：从新模验证到旧模报废（SCR/TDA）流转不够紧密。（2026-09-22 评审决定：旧模处置移出系统范围，第 5/6 步删除，见 v2 提案）
...
- 固化标准 4 阶段流水线（立项 -> 开发制造 -> 内部审批 -> 客户审批）与前置依赖网络（2026-09-22 评审决定删除报废处置与归档两阶段）；
- 提供组合看板（Portfolio）、项目详情（Project）、个人待办（My Actions）、管理看板（Management View）及新建向导（Wizard）；
- 具备数据库持久化与 Excel 双向无缝互通能力。

## Goals
- **GOAL_1**: 完整支撑 4 阶段全生命周期与 6 类跨职能角色的协同任务闭环。
...
| [v2-review-feedback](roadmap/v2-review-feedback.md) | `planned` |
