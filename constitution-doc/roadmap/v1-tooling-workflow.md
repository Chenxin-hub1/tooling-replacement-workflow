...
2026-09-22 发布授权：用户明确要求“上传并更新到 GIT”，覆盖前文暂不上传的历史限制。本次将已验证代码、部署说明及重导后的本地数据库提交并推送既有 origin/main；服务器由用户按部署说明更新。

2026-09-23 误触保护（用户授权）：Admin 的 "Reset to demo data" 在工作区存在演示之外的项目（含真实导入数据）时，confirm 明示项目规模与后果，并在重置前自动调用 `saveWorkspaceFile()` 下载当前工作区 JSON 作本地备份（`workspace.js` 的 `resetDemo`），配合既有服务器滚动快照双通道可恢复。e2e 新增两分支用例（纯演示工作区不警告不备份、含非演示项目警告并备份）。

2026-09-24 日期格修复（整体核查发现，用户授权修复）：浏览器导入真实 Excel 日期单元格时，SheetJS `cellDates` 给出本地零点 Date，`isoDate`/`sourceText` 经 `toISOString()` 在东八区早一天（2026-09-23 记成 2026-09-22）。修复为 `importExcel` 的 `sheet_to_json` 加 `UTC:true`（`improvements.js`），e2e 在 Asia/Taipei 与 America/Los_Angeles 两个时区各验证一次（修复前东八区用例失败复现，负时区本就通过）。已知：真实库 46 条项目原始行中 55 个 ISO 形态日期格于 2026-09-18 在 Windows 浏览器导入，很可能整体早一天；源表在用户 OneDrive 桌面，需对照核对后重导或人工修正，尚待用户决定。

2026-09-24 源表重导（用户指示）：日期格修复后用源表经真实页面重导 46 条（按行号注入原 Project ID，创建日期保持 2026-09-18），修订 5 → 6；55 个日期格与源表逐一相等，31 处动作日期 +1 天，其余差异仅 `\r\n` → `\n`，完成动作 5 → 5，编号/依赖/创建日期不变。待处理：16 个双日期动作的原始日期仍为旧偏移值需 Admin 修正留痕；工具身份哈希对换行符敏感，直接导源表会把 6 行当新项目。记录见 `docs/date-reimport-2026-09-24.md`。

2026-09-24 原始日期修正（用户执行）：重导后 16 个双日期动作的 `orig` 仍为旧偏移值，经 `frontend/scripts/fix-orig-dates-2026-09-24.mjs` 按 Admin 修正路径批量改为当前值并在 `origLog` 留痕，修订 6 → 7；数据库主文件已合并 WAL。剩余待办：工具身份哈希对换行符敏感（直接导源表 6 行会成新项目），以及本次全部改动尚未提交。

2026-09-24 筛选空值修正（用户授权）：真实数据没有 BU 列，Portfolio 的 BU 下拉出现与"未选择"同值的空选项，框内显示空白。`prepare-prototype.mjs` 新增两条替换：`uniq` 与供应商列表 `.filter(Boolean)` 剔除空值；e2e 新增用例（BU 全空时只剩占位项，供应商列表无空项）。
