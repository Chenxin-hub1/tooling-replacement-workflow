...
本次把仓库内已重新核验的 46 项目数据库装入数据卷（2026-09-23：BPW/CVS CR/OEM - Tech 映射落地重导后，样板演示项目已删，仅剩真实导入数据）。服务器当前数据会完整备份；若服务器另有需要合并的新编辑，先核对这些编辑再执行数据库替换。后续只更新代码时，省略数据库安装步骤即可保留卷内当前数据。
...
`--wait` 等待容器健康；较旧 Compose 若不支持，应升级 Compose，或使用 `up -d` 后自行确认 health。正常应看到 `healthy`，浏览器访问 `http://服务器IP:8000` 并刷新页面。数据核对：46 项目（全部真实导入，无演示项目）、5 项完成、Core BPW 有值 12、OEM BPW 有值 1、CVS CR 有值 4、主数据 `tech` 全部 46、initiated 状态 61。端口如被服务器覆盖，以实际映射为准。
