...
replaceOnce(
  "(a.value?'background:var(--g-soft);",
  "(a.done?'background:var(--g-soft);",
);
...
function replaceExact(oldText, newText, expected) {
  ...
  if (count !== expected)
    throw new Error(
      `Reference changed: ${count}x instead of ${expected}x -- ${oldText}`,
    );
...
const escRules = [
  ...
  [
    '${s.next.tab}<br><span class="muted">${fmt(s.next.due)}</span>',
    ["s.next.tab"],
    1,
  ],
  [
    '<h1>${p.id} · ${p.desc}</h1><div class="sub">${p.pn} · ${p.plant} · ${p.bu} · created',
    ["p.id", "p.desc", "p.pn", "p.plant", "p.bu"],
    1,
  ],
  ["next: ${s.next.tab} ${fmt(s.next.due)}", ["s.next.tab"], 1],
  [
    '— ${a.tab} · ${a.act} <span class="muted">(${a.owner})</span>',
    ["a.tab", "a.act", "a.owner"],
    1,
  ],
  ...
  [
    '<td>${a.fn}</td><td>${a.owner}<br><span class="muted">${email(a.owner)}</span></td>',
    ["a.fn", "a.owner", "email(a.owner)"],
    1,
  ],
  ...
  [
    '<td><b>${p.id}</b><br><span class="muted">${p.desc}</span></td>',
    ["p.id", "p.desc"],
    1,
  ],
  [
    'const hb=(l,n,sub)=>`<div class="hbar"><span>${l}${sub?`<br><span class="hint">${sub}</span>`:\'\'}</span>',
    ["l", "sub"],
    1,
  ],
  ["<b>${m.s}</b>", ["m.s"], 1],
  [
    "→ ${m.to} &lt;${email(m.to)}&gt;${m.cc?` · Cc: ${m.cc} (project owner)`:''}",
    ["m.to", "email(m.to)", "m.cc"],
    1,
  ],
  [
    "<b>${m.a.tab} · ${m.a.act} (${m.p.id})</b>",
    ["m.a.tab", "m.a.act", "m.p.id"],
    1,
  ],
  [
    "→ ${m.to} &lt;${email(m.to)}&gt;${m.cc?` · Cc: ${m.cc}`:''} · previewed by ${m.by}",
    ["m.to", "email(m.to)", "m.cc", "m.by"],
    1,
  ],
  ["<h2>${a.tab} · ${a.act}</h2>", ["a.tab", "a.act"], 1],
  [
    "${p.id} · ${PHASES[a.ph]} · owner ${a.owner} · ${a.due?'due '",
    ["p.id", "a.owner"],
    1,
  ],
  ...
  [
    "${p.id} · ${a.tab} · ${a.act} · ${a.due?(dl<0?",
    ["p.id", "a.tab", "a.act"],
    1,
  ],
  ["→ ${a.owner} &lt;${email(a.owner)}&gt;", ["a.owner", "email(a.owner)"], 1],
  [
    "Hi ${a.owner.split(' ')[0]}, quick reminder on ${p.id} (${p.desc}): \"${a.act}\" ${a.due?(dl<0?`was due",
    ["a.owner.split(' ')[0]", "p.id", "p.desc", "a.act"],
    1,
  ],
  [
    "Please enter the ${a.input} in the dashboard. Thanks, ${me}",
    ["a.input", "me"],
    1,
  ],
  ...
  [
    '<option value="${a.id}">${a.tab} · ${a.act.slice(0,40)}</option>',
    ["a.id", "a.tab", "a.act.slice(0,40)"],
    1,
  ],
  ["value=\"${W.d[k]||''}\"", ["W.d[k]||''"], 1],
  ...
  [
    "${p.team['BU Buyer']}<br><span>${email(p.team['BU Buyer'])}</span>",
    ["p.team['BU Buyer']", "email(p.team['BU Buyer'])"],
    1,
  ],
  ["<span>${email(p.team[f])}</span>", ["email(p.team[f])"], 1],
  [
    '<tr><td>${n}</td><td>${f}</td><td class="muted">${email(n)}</td>',
    ["n", "f", "email(n)"],
    1,
  ],
  ["<option>${f}</option>", ["f"], 1],
...
const escRewrites = [
  [
    "value=\"${String(p[k]||'').replace(/\"/g,'&quot;')}\"",
    "value=\"${escHtml(String(p[k]||''))}\"",
    1,
  ],
  [
    "value=\"${(a.comment||'').replace(/\"/g,'&quot;')}\"",
    "value=\"${escHtml(a.comment||'')}\"",
    1,
  ],
  [
  ...
  [
    "${a.value?'<b>'+a.value+'</b>':a.input}",
    "${a.value?'<b>'+escHtml(a.value)+'</b>':escHtml(a.input)}",
    1,
  ],
  [
    "<span class=\"hint\">${W.team[f]?email(W.team[f]):'&nbsp;'}</span>",
    "<span class=\"hint\">${W.team[f]?escHtml(email(W.team[f])):'&nbsp;'}</span>",
    1,
  ],
  [
    "<td>${a.owner||'<span class=\"req\">unassigned</span>'}</td>",
    "<td>${a.owner?escHtml(a.owner):'<span class=\"req\">unassigned</span>'}</td>",
    1,
  ],
  [
    "${p[k]||'<span class=\"muted\">—</span>'}",
    "${p[k]?escHtml(p[k]):'<span class=\"muted\">—</span>'}",
    1,
  ],
  [
    "<span>${l}</span>${p.team[f]||'<span class=\"req\">not assigned</span>'}",
    "<span>${l}</span>${p.team[f]?escHtml(p.team[f]):'<span class=\"req\">not assigned</span>'}",
    1,
  ],
];
...
// 筛选下拉剔除空值：真实数据没有 BU 列时，空选项会与"未选择"同值，框内显示空白（2026-09-24）。
replaceOnce(
  "const uniq=k=>[...new Set(projects.map(p=>p[k]))];",
  "const uniq=k=>[...new Set(projects.map(p=>p[k]))].filter(Boolean);",
);
replaceOnce(
  "${[...new Set(projects.flatMap(p=>[p.cur,p.nw]))].map(",
  "${[...new Set(projects.flatMap(p=>[p.cur,p.nw]))].filter(Boolean).map(",
);
...
replaceOnce(
  ...
  "let phase=PHASES.length-1; for(let i=0;i<PHASES.length;i++)",
);
